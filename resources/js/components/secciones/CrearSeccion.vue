<template>
    <v-app class="container">
        <div class="row mt-5">
            <div class="col-md-12"> 
                <div class="card-tools">                            
                    <button class="btn btn-success" @click="newModal" data-toggle="modal" data-target="#addNew">
                        Añadir Sección
                        <i class="fas fa-user-plus fa-fw"></i>
                    </button>
                </div>        
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Secciones</h3>      
                        <div class="card-tools">
                            <v-text-field
                                v-model="search"
                                append-icon="mdi-magnify"
                                label="buscar"
                                single-line
                                hide-details
                            ></v-text-field>                            
                        </div>              
                    </div>
                    <!-- /.card-header -->
                    <v-card>                        
                        <v-data-table
                            :headers="headers"
                            :items="secciones"
                            :search="search"
                        >   
                            <template v-slot:item.foto="{ item }">
                                <div class="p-2">
                                    <v-img :src="'img/secciones/'+item.foto" :alt="item.foto" width="50px" height="50px"></v-img>
                                </div>
                            </template>
                            <template v-slot:item.accion="{ item }">
                                <a href="#">
                                    <i class="fa fa-edit" @click="editModal(item)"></i>
                                </a>
                                /
                                <a href="#" @click="deleteSeccion(item.id)">
                                    <i class="fa fa-trash" style="color: red"></i>
                                </a>
                            </template>
                        </v-data-table>
                    </v-card>                    
                </div>
                <!-- /.card -->
            </div>
        </div>

        <!-- Modal -->
        <div class="modal fade" id="addNew" tabindex="-1" role="dialog" aria-labelledby="addNewLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" v-show="!editmode" id="addNewLabel">Crear Sección</h5>
                        <h5 class="modal-title" v-show="editmode" id="addNewLabel">Actualizar info</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form @submit.prevent="editmode ? updateSeccion() : createSeccion()">
                        <div class="modal-body">
                            <div class="form-group">
                                <v-text-field 
                                    label="Nombre"  
                                    v-model="form.nombre_seccion" 
                                    type="text" 
                                    name="nombre_seccion"                                    
                                    prepend-icon="fa-solid fa-square-pen"                               
                                    :class="{ 'is-invalid': form.errors.has('nombre_seccion') }">
                                </v-text-field>
                                <has-error :form="form" field="nombre_seccion"></has-error>
                            </div>                                                                                                                        

                            <div class="form-group">
                                <v-textarea 
                                    label="Descripción" 
                                    v-model="form.descripcion_seccion" 
                                    name="descripcion_seccion" 
                                    id="descripcion_seccion"                                    
                                    value="Ingrese una palabra clave"
                                    prepend-icon="fa-solid fa-file-lines"
                                    :class="{ 'is-invalid': form.errors.has('descripcion_seccion') }">
                                </v-textarea>
                                <has-error :form="form" field="descripcion_seccion"></has-error>
                            </div>  

                            <div class="form-group">                                
                                <v-select 
                                    item-text="nombre_carrera"
                                    item-value="id"
                                    v-model.number="form.id_carrera"
                                    id="select_carrera" 
                                    :items="carreras"
                                    label="Carrera a la que Pertenece"                                    
                                    prepend-icon="fa-solid fa-home"
                                    name="id_carrera"
                                    :class="{ 'is-invalid': form.errors.has('id_carrera') }">
                                </v-select>
                                <has-error :form="form" field="id_carrera"></has-error>
                            </div>

                            <div class="form-group">                                
                                <v-select 
                                    item-text="nombre_categoria"
                                    item-value="id"
                                    v-model.number="form.id_categoria"
                                    id="select_categoria" 
                                    :items="categorias"
                                    label="Categoria a la que Pertenece"                                    
                                    prepend-icon="fa-solid fa-home"
                                    name="id_categoria"
                                    :class="{ 'is-invalid': form.errors.has('id_categoria') }">
                                </v-select>
                                <has-error :form="form" field="id_categoria"></has-error>
                            </div>

                            <div class="form-group">
                                <v-text-field 
                                    label="Enlace"  
                                    v-model="form.enlace_seccion" 
                                    type="text" 
                                    name="enlace_seccion"                                    
                                    prepend-icon="fa-solid fa-square-pen"                               
                                    :class="{ 'is-invalid': form.errors.has('enlace_seccion') }">
                                </v-text-field>
                                <has-error :form="form" field="enlace_seccion"></has-error>
                            </div> 

                            <div class="form-group">                                
                                <v-file-input  
                                    v-model="foto"                                    
                                    accept="image/*"
                                    show-size
                                    counter
                                    label="Foto" 
                                    @change="fotoSeccion" 
                                    name="foto" 
                                    prepend-icon="mdi-camera"
                                    :class="{ 'is-invalid': form.errors.has('foto') }">
                                </v-file-input>    
                                <has-error :form="form" field="foto"></has-error>                                
                            </div>

                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-dismiss="modal">Cerrar</button>
                            <button v-show="editmode" type="submit" class="btn btn-success">Actualizar</button>
                            <button v-show="!editmode" type="submit" class="btn btn-primary">Crear</button>
                        </div>
                    </form> 
                </div>
            </div>
        </div>
    </v-app>
</template>

<script>    
    export default {          
        data() {
            return {                
                editmode: false,                
                carreras : [],
                categorias : [],
                foto: [],
                form: new Form({ 
                    id : '',  
                    nombre_seccion : '',                    
                    descripcion_seccion : '', 
                    id_carrera : '',
                    id_categoria : '',                     
                    enlace_seccion : '',  
                    foto : '',                                     
                }),
                search: '',
                headers: [
                    {
                        text: 'Nombre',
                        align: 'start',
                        sortable: false,
                        value: 'nombre_seccion',
                    },                    
                    { text: 'Descripción', value: 'descripcion_seccion' },                    
                    { text: 'Carrera', value: 'nombre_carrera' },                    
                    { text: 'Categoria', value: 'nombre_categoria' },                    
                    { text: 'Foto', value: 'foto' },                              
                    { text: 'Enlace', value: 'enlace_seccion' },      
                    { text: 'Acciones', value: 'accion' },
                ],
                secciones : [],
            }
        },
        mounted() {
            //console.log(this.form)
        },
        methods: { 
            updateSeccion(){
                this.$Progress.start();
                // console.log('Editing data');
                this.form.put('api/secciones/'+this.form.id)
                .then(() => {
                    // success
                    $('#addNew').modal('hide');
                    Swal.fire(
                        'Actualizado!',
                        'Informacion Actualizada Correctamente.',
                        'success'
                        )
                        this.$Progress.finish();
                        Fire.$emit('AfterCreate');
                    })
                .catch(() => {
                    this.$Progress.fail();
                });
            },
            editModal(seccion){
                this.editmode = true;
                this.form.reset();
                $('#addNew').modal('show');
                this.form.fill(seccion);
                this.foto = new File([""], this.form.foto, {type: "text/plain", lastModified: new Date().toString()});
            },
            newModal(){
                this.editmode = false;
                this.form.reset();
                this.foto = [];
                $('#addNew').modal('show');
            },
            deleteSeccion(id){
                Swal.fire({
                    title: 'Estas Seguro?',
                    text: "Esta accion no se puede revertir!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Si, Borrar!'
                    }).then((result) => {
                    if (result.isConfirmed) {
                        this.form.delete('api/secciones/'+id).then(()=>{
                            Swal.fire(
                            'Eliminada!',
                            'La Seccion ha sido Eliminada.',
                            'success'
                            )
                            Fire.$emit('AfterCreate');
                        }).catch(()=> {
                            Swal.fire("Error!", "Ha ocurrido algun Problema.", "warning");
                        });    
                    }
                })
            },                                                              
            async loadSeccion(){   
                if(this.$gate.isAdmin()){                             
                    await axios.get("api/secciones")
                        .then(response =>{
                            this.secciones = response.data;
                        })
                        .catch(error=>{
                            this.secciones = []
                        });
                }
            },    
            async loadCarrera(){                                               
                await axios.get("api/carreras")
                    .then(response =>{
                        this.carreras = response.data;
                    })
                    .catch(error=>{
                        this.carreras = []
                    });                                                                                           
            },   
            async loadcategoria(){   
                if(this.$gate.isAdmin()){                             
                    await axios.get("api/categorias")
                        .then(response =>{
                            this.categorias = response.data;
                        })
                        .catch(error=>{
                            this.categorias = []
                        });
                }
            },  
            fotoSeccion(e){      
                //console.log(e)   ;                      
                //let file = e.target.files[0];
                if(e != null){
                    let reader = new FileReader();  
                    let size = e.size;              
                    //console.log(reader);
                    let limit = 1024 * 1024 * 2;
                    if(size > limit){
                        Swal.fire({
                            type: 'error',
                            title: 'Oops...',
                            text: 'El archivo es muy pesado!',
                        })
                        return false;
                    }
                    reader.onloadend = (e) => {
                        //console.log(reader.result);
                        this.form.foto = reader.result;
                    }
                    reader.readAsDataURL(e);
                }else{
                    console.log("Se debe agregar una foto");
                }
                
            },                 
            createSeccion(){
                this.$Progress.start();
                this.form.post('api/secciones')
                .then(()=>{
                    Fire.$emit('AfterCreate');
                    $('#addNew').modal('hide')
                    Toast.fire({
                    icon: 'success',
                    title: 'Sección Creada Satisfactoriamente'
                })
                this.$Progress.finish();
                })
                .catch(()=>{
                })
            },
            
        },
        created() {
            this.loadSeccion();
            this.loadCarrera();
            this.loadcategoria();
            Fire.$on('AfterCreate',() => {
               this.loadSeccion();
            }); 
        }
    }
</script>

